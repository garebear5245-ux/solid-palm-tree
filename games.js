/* ============================================================================
   ...
   Each item has a `tags: { … }` block. Set any to true to show its badge:
     achievements · offline · multiplayer · controller · nes · doodle
     broken  → red "⚠ May be broken" badge
     fixed   → green "🛠 Fixed" badge  (use for a game you rebuilt/repaired so
               it now works — e.g. made to run fully offline on a Chromebook)

   ── Making an emulated/online game work OFFLINE on a Chromebook (the Kirby fix) ─
   Chromebooks download the .zip, extract it, and open index.html from file://.
   Two things break the old EmulatorJS games there:
     1. They pull the emulator + ROM from cdn.jsdelivr.net — school filters block it.
     2. They use fetch()/XHR, which Chrome REFUSES on file:// origins.
   The fix (used for Kirby's Adventure) makes the zip fully self-contained:
     • Use jsnes (pure-JS NES emulator, no WASM) instead of EmulatorJS.
     • Put jsnes.min.js in the zip and load it with <script src> (works on file://).
     • Base64 the ROM into a rom.js as `window.<NAME>_ROM_B64="…"` and load it with
       <script src> too — NEVER fetch() it.  (script tags are allowed on file://.)
     • index.html decodes the ROM with atob(), calls nes.loadROM(binaryString),
       renders to a <canvas>, and drives audio with a WebAudio ScriptProcessor.
   Result: no network, no CDN, no fetch → runs on the most locked-down Chromebook.
   Then set  offline: true  and  fixed: true  on that game's tags.
   ============================================================================ */
    ...
    items: [
        {
            ...
            tags: {
                ...
                controller:   false,   // supports a gamepad
                  ...
                  {
            name:        'CrazyCattle 3D',
            file: 'Q3JhenlDYXR0bGUzRC56aXA=', // btoa('CrazyCattle3D.zip')
            category:    'Game',
            type:        'Arcade',
            cardText:    'Ram rival sheep off the arena.',
            description: 'A game where you ram sheep and battle to be the last one standing.',
            tags: {
                achievements: false,    // has in-game achievements / medals
                offline:      true,    // playable with no internet
                broken:       false,   // known to be buggy / unfinished
                ...
                controller:   false,   // supports a gamepad
                  ...
                  {
            name:        'Slow Roads',
            file: 'c2xvd3JvYWRzLnppcA==', // btoa('slowroads.zip')
            category:    'Game',
            type:        'Driving',
            cardText:    'Drive around on windy roads for fun.',
            description: 'Drive around on windy roads for fun.',
            tags: {
                achievements: false,    // has in-game achievements / medals
                offline:      true,    // playable with no internet
                broken:       false,   // known to be buggy / unfinished
                ...
                controller:   false,   // supports a gamepad
                  ...
                  {
            name:        'Kirbys Adventures',
            file: 'S2lyYnlzQWR2ZW50dXJlcy56aXA=', // btoa('KirbysAdventures.zip')
            category:    'Game',
            type:        'NES',
            cardText:    'Play as Kirby and defend Dream Land!',
            description: 'Play as Kirby and defend Dream Land! Credits: Nintendo.',
            tags: {
                achievements: false,    // has in-game achievements / medals
                offline:      true,    // playable with no internet
                broken:       false,   // known to be buggy / unfinished
                multiplayer:  false,   // has a multiplayer mode
                controller:   false,   // supports a gamepad
                nes:          true,    // NES emulated game
                fixed:        true,    // rebuilt to run fully offline (jsnes) — works on locked-down Chromebooks
            },
        },

                  {
            name:        'Drift Boss',
            file: 'RHJpZnRCb3NzLnppcA==', // btoa('DriftBoss.zip')
            category:    'Game',
            type:        'Driving',
            cardText:    'Keep the car on the path as long as you can.',
            description: 'A game where you are supposed to keep the car on the path for as long as possible.',
            tags: {
                achievements: false,    // has in-game achievements / medals
                offline:      true,    // playable with no internet
                broken:       false,   // known to be buggy / unfinished
            ...
            file: 'R29vZ2xlQmFzZWJhbGwuemlw', // btoa('GoogleBaseball.zip')
            category:    'Game',
            type:        'Google Doodles',
            cardText:    'Hit the baseball and see how far it goes!',
            description: 'The google doodle baseball game. Hit the baseball at the right time ' +
                         'and see how far it will go! Credits: Google Doodles.',
            tags: {
                achievements: false,    // has in-game achievements / medals
                offline:      true,    // playable with no internet
                broken:       false,   // known to be buggy / unfinished
                multiplayer:  false,   // has a multiplayer mode
                controller:   false,   // supports a gamepad
                doodle:       true,    // Google Doodle game
            },
        },
        ...
        /* ── Copy this block to add another game or app ──────────────────────
        {
            ...
            tags: { achievements: false, offline: true, broken: false, multiplayer: false, controller: false, fixed: false },
        },
        */
    ],
};
