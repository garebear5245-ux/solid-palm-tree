...
(function () {
    ...
    const TAG_META = {
        ...
        nes:          { label: 'NES game',         icon: '🎮' },
        doodle:       { label: 'Google Doodle',    icon: '✎' },
        fixed:        { label: 'Fixed / verified working', icon: '🛠' },
    };
    ...
    function miniBadges(item) {
        ...
        const t = item.tags || {};
        let html = '';
        if (t.fixed)        html += `<span class="mini-badge good">🛠 Fixed</span>`;
        ...
        if (t.nes)          html += `<span class="mini-badge">🎮 NES</span>`;
        if (t.doodle)       html += `<span class="mini-badge">✎ Google Doodle</span>`;
    ...
    function matches(item) {
        ...
        switch (activeFilter) {
            ...
            case 'nes':          return item.tags && item.tags.nes;
            case 'doodle':       return item.tags && item.tags.doodle;
            default:             return true;
        }
    }
    ...
    const SECTIONS = [
        ...
        { id: 'nes',      title: 'NES',      match: it => !it.isStore && !it.isAC && it.tags && it.tags.nes },
        { id: 'doodles',  title: 'Google Doodles', match: it => !it.isStore && !it.isAC && it.tags && it.tags.doodle },
